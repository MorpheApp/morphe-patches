 ...
 * identified by an IGET_OBJECT of the MusicActivity peer's AtomicBoolean.
 */
...
/**
 * Matches the cold start miniplayer text.
 * Caller must supply the method string resolved from [MiniPlayerDefaultTextFingerprint].
 */
internal fun coldStartMiniPlayerDefaultTextFingerprint(miniPlayerDefaultTextMethod: String) = Fingerprint(
    accessFlags = listOf(AccessFlags.PUBLIC, AccessFlags.FINAL),
    parameters = listOf("Ljava/lang/Object;"),
    returnType = "V",
    filters = listOf(
        ...
        methodCall(
            opcode = Opcode.INVOKE_VIRTUAL,
            smali = miniPlayerDefaultTextMethod
        )
    )
)

/**
 * Matches the warm start miniplayer.
 * Caller must supply the dynamic classes via [WatchWhileLayoutFingerprint].
 */
internal fun warmStartMiniplayerFingerprint(
    warmStartMiniplayerClass: String,
    watchWhileLayoutClass: String) = Fingerprint(
