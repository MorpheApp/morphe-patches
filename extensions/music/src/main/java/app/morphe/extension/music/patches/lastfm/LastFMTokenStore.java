...
public class LastFMTokenStore {
    private static final String PREFS_NAME = "lastfm_token";
    ...
    public static synchronized void init(Context context) {
        ...
        try {
            ...
            Logger.printException(() -> "LastFMTokenStore init failed", e);
        }
    }
    ...
    private static String encrypt(String value) {
        ...
        try {
            ...
            Logger.printException(() -> "LastFMTokenStore: encrypt failed", e);
            return null;
        }
    }
    ...
    private static String decrypt(String value) {
        ...
        try {
            ...
            Logger.printException(() -> "LastFMTokenStore: decrypt failed", e);
            return null;
        }
    }
    ...
    public static boolean store(String sessionKey, String username) {
        ...
        if (p == null) {
            Logger.printInfo(() -> "LastFMTokenStore: cannot store, prefs not initialized");
            return false;
        }
        String encryptedSessionKey = encrypt(sessionKey);
        if (encryptedSessionKey == null) {
            Logger.printInfo(() -> "LastFMTokenStore: encryption failed, not persisting token");
            return false;
        }
        try {
            return p.edit()
                    .putString(SESSION_KEY_KEY, encryptedSessionKey)
                    .putString(USERNAME_KEY, username)
                    .commit();
        } catch (Exception e) {
            Logger.printException(() -> "LastFMTokenStore: failed to store credentials", e);
            return false;
        }
    }
    ...
    public static String retrieveSessionKey() {
        ...
        String encrypted = p.getString(SESSION_KEY_KEY, null);
    ...
    public static String retrieveUsername() {
        SharedPreferences p = getPrefs();
        if (p == null) return null;
        return p.getString(USERNAME_KEY, null);
    }

    public static boolean clear() {
        SharedPreferences p = getPrefs();
        if (p == null) {
            Logger.printInfo(() -> "LastFMTokenStore: cannot clear, prefs not initialized");
            return false;
        }
        try {
            return p.edit()
                    .remove(SESSION_KEY_KEY)
                    .remove(USERNAME_KEY)
                    .commit();
        } catch (Exception e) {
            Logger.printException(() -> "LastFMTokenStore: failed to clear credentials", e);
            return false;
        }
    }
    ...
    private static class AesKeystore {
        ...
        private static final String KEY_ALIAS = "morphe_lastfm_token_key";
